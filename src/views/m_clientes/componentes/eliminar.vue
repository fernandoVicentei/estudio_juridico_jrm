<template>
  <div class="row" >
   <div class="col-md-12" >
       <strong>El registro que fue seleccionado no puede ser eliminado. ¿Desea inhabilitarlo?</strong>
   </div>
   <div class="col-md-12 mt-3 mb-3 d-flex justify-content-end" >
     ...
     <button class="btn btn-info text-light" type="submit"  @click="desabilitar()" >Guardar</button>
   </div>
  </div>
...
export default {
 ...
 methods:{
     desabilitar(){
         let obj= { 'idCliente': this.idAbg }
         postFetch( ruta_servidor +'clientes/eliminar' , obj )
         .then((res)=>{
             if(res.status == 422){
               ...
               this.abrirError()
                ...
                if(res.success){
                   this.mensaje  = 'Registro desabilitado correctamente'
                   this.abrirOk()
                }else{
